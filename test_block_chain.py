...
from libs import actions, tools, check, db, api, contract
...
class TestBlockChain():
    ...
    def test_block_chain_edit(self):
        ts_count = 100
        tx = contract.new_menu(self.config1["url"], self.config1['private_key'], self.token)
        check.is_tx_in_block(self.config1["url"], self.wait, tx, self.token)
        id = actions.get_object_id(self.config1["url"], tx['name'], "menu", self.token)
        time.sleep(10)
        i = 1
        amounts_b = actions.get_user_token_amounts(self.config1["url"], self.token)
        sum_amounts_before = sum(amounts_b)
