...
class TestLimits():
    ...
    def assert_tx_in_block(self, result, jwtToken):
        ...
        hash = result['hash']
        status = actions.tx_status(self.conf["2"]["url"], self.wait, hash, self.token)
    ...
    def call(self, name, data):
        resp = actions.call_contract(self.conf["2"]["url"], self.conf["1"]['private_key'],
    ...
    def test_max_tx_size(self):
        max_tx_size = db.get_system_parameter(self.conf["1"]["db"], "max_tx_size")
    ...
    def test_max_block_size(self):
        max_block_size = db.get_system_parameter(self.conf["1"]["db"], "max_block_size")
    ...
    def test_max_block_user_tx(self):
        max_block_user_tx = db.get_system_parameter(self.conf["1"]["db"], "max_block_user_tx")
        ...
        while i < 10: 
            ...
            data = {"Wallet": "", "Value": code, "ApplicationId": 1,
                "Conditions": "true"}
            actions.call_contract(self.conf["2"]["url"], self.conf["1"]['private_key'],
                                "NewContract", data, self.token)
            i = i + 1
        time.sleep(5)
        maxBlock = actions.get_max_block_id(self.conf["2"]["url"], self.token)
        print("maxBlock = ", maxBlock)
        isOneOrTwo = db.is_count_tx_in_block(self.conf["2"]["db"], maxBlock, 1)
    ...
    def test_max_tx_count (self):
        max_tx_count = db.get_system_parameter(self.conf["1"]["db"], "max_tx_count")
        ...
        time.sleep(5)
        maxBlock = actions.get_max_block_id(self.conf["2"]["url"], self.token)
        self.unit.assertTrue(db.is_count_tx_in_block(self.conf["2"]["db"], maxBlock, 2),
                        "One of block contains more than 2 transaction")
