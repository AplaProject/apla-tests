...
from libs import actions, db, tools, api, contract as contracts, check
...
class TestSimvolio():
    ...
    def test_sys_vars_readonly(self):
        ...
        for i in range(len(var_list)):
            code = " { data{ } conditions{ } action{ " + var_list[i] + " = 5 } }"
            tx = contracts.new_contract(self.url, self.pr_key, self.token, source=code)
    ...
    def test_z1_db_select_metrics_min(self):
        ...
        code2 = " {\n data{} \n conditions{} \n action { \n  $result = $block \n } \n }"
        tx2 = contracts.new_contract(self.url, self.pr_key, self.token, source=code2)
    ...
    def test_get_history_page(self):
        # create page
        page = "Div(Body: Hello)"
        tx_page = contracts.new_page(self.url, self.pr_key, self.token, value=page)
        ...
        tx = contracts.edit_page(self.url, self.pr_key, self.token, id, value=new_value_page)
        check.is_tx_in_block(self.url, self.wait, tx, self.token)
        # test
    ...
    def test_get_history_row_menu(self):
        ...
        menu = contracts.new_menu(self.url, self.pr_key, self.token, value=value)
    ...
    def test_get_history_row_block(self):
        ...
        block = contracts.new_block(self.url, self.pr_key, self.token, value=value)
