...
class TestCost():
    ...
    def setup(self):
        print("setup")
        self.data = actions.login(self.conf["2"]["url"], self.keys["key2"], 0)
    ...
    def getNodeBalances(self):
        ...
        nodeBalance = []
        while i < nodeCount + 1:
            nodeBalance.append(db.get_balance_from_db(self.conf["1"]["db"],
                                                      self.conf[str(i)]["keyID"]))
            i = i + 1
        return nodeBalance
    ...
    def createContracts():
        global dataCreater
        dataCreater = actions.login(TestCost.conf["1"]["url"], TestCost.conf["1"]["private_key"], 0)
        ...
        result = actions.call_contract(TestCost.conf["1"]["url"], TestCost.conf["1"]["private_key"],
                                       "NewContract", data, tokenCreater)
        status = actions.tx_status(TestCost.conf["1"]["url"], TestCost.wait,
                                   result['hash'], tokenCreater)

    def activateContract(self):
        dataCreater = actions.login(self.conf["2"]["url"], self.conf["1"]["private_key"], 0)
        tokenCreater = dataCreater["jwtToken"]
        id = actions.get_contract_id(self.conf["2"]["url"], "CostContract", tokenCreater)
        data = {"Id": id}
        result = actions.call_contract(self.conf["2"]["url"], self.conf["1"]["private_key"],
                                       "ActivateContract", data, tokenCreater)

        status = actions.tx_status(self.conf["2"]["url"], self.wait,
                                   result['hash'], tokenCreater)

    def deactivateContract(self):
        dataCreater = actions.login(self.conf["1"]["url"], self.conf["1"]["private_key"], 0)
        tokenCreater = dataCreater["jwtToken"]
        id = actions.get_contract_id(self.conf["1"]["url"], "CostContract", tokenCreater)
        data = {"Id": id}
        result = actions.call_contract(self.conf["1"]["url"], self.conf["1"]["private_key"],
                                       "DeactivateContract", data, tokenCreater)
        status = actions.tx_status(self.conf["1"]["url"], self.wait, result['hash'], tokenCreater)

    def isCommissionsInHistory(self, nodeCommision, idFrom, platformaCommission, node):
        isNodeCommission = db.is_commission_in_history(self.conf["1"]["db"], idFrom,
                                                         self.conf[str(node + 1)]["keyID"],
                                                         nodeCommision)
        isPlatformCommission = db.is_commission_in_history(self.conf["1"]["db"], idFrom,
                                                             self.conf["1"]["keyID"],
                                                             platformaCommission)
    ...
    def test_activated_contract(self):
        if actions.is_contract_activated(self.conf["2"]["url"], "CostContract", self.token) == False:
            self.activateContract()
        time.sleep(10)
        walletId = actions.get_activated_wallet(self.conf["2"]["url"],
                                                "CostContract", self.token)
        sumsBefore = db.get_user_token_amounts(self.conf["1"]["db"])
        summBefore = sum(summ[0] for summ in sumsBefore)
        bNodeBalance = self.getNodeBalances()
        tokenRunner, uid = actions.get_uid(self.conf["2"]["url"])
        signature = sign(self.keys["key2"], uid)
        pubRunner = get_public_key(self.keys["key2"])
        balanceRunnerB = db.get_balance_from_db_by_pub(self.conf["1"]["db"], pubRunner)
        dataRunner = actions.login(self.conf["2"]["url"], self.keys["key2"], 0)
        tokenRunner = dataRunner["jwtToken"]
        res = actions.call_contract(self.conf["2"]["url"], self.keys["key2"],
                                    "CostContract", {"State": 1}, tokenRunner)
        result = actions.tx_status(self.conf["2"]["url"], self.wait, res["hash"], tokenRunner)
        time.sleep(10)
        node = db.get_block_gen_node(self.conf["1"]["db"], result["blockid"])
        sumsAfter = db.get_user_token_amounts(self.conf["1"]["db"])
        ...
        balanceRunnerA = db.get_balance_from_db_by_pub(self.conf["1"]["db"], pubRunner)
        balanceContractOwnerA = db.get_balance_from_db(self.conf["1"]["db"], walletId)
        inHistory = self.isCommissionsInHistory(nodeCommission, self.conf["1"]["keyID"],
    ...
    def test_deactive_contract(self):
        if actions.is_contract_activated(self.conf["2"]["url"], "CostContract", self.token) == True:
    ...
    def test_activated_contract_with_err(self):
        if actions.is_contract_activated(self.conf["2"]["url"], "CostContract", self.token) == False:
            self.activateContract()
        walletId = actions.get_activated_wallet(self.conf["2"]["url"], "CostContract", self.token)
        balanceContractOwnerB = db.get_balance_from_db(self.conf["1"]["db"], walletId)
        balanceNodeOwnerB = db.get_balance_from_db(self.conf["1"]["db"], self.conf["2"]["keyID"])
        commisionWallet = db.get_commission_wallet(self.conf["1"]["db"], 1)
        balanceCommisionB = db.get_balance_from_db(self.conf["1"]["db"], commisionWallet)
        tokenRunner, uid = actions.get_uid(self.conf["2"]["url"])
        signature = sign(self.keys["key2"], uid)
        pubRunner = get_public_key(self.keys["key2"])
        balanceRunnerB = db.get_balance_from_db_by_pub(self.conf["1"]["db"], pubRunner)
        ...
        hash = res["hash"]
        result = actions.tx_status(self.conf["2"]["url"], self.wait, hash, tokenRunner)
        time.sleep(10)
        balanceContractOwnerA = db.get_balance_from_db(self.conf["1"]["db"], walletId)
        balanceNodeOwnerA = db.get_balance_from_db(self.conf["1"]["db"], self.conf["2"]["keyID"])
        balanceCommisionA = db.get_balance_from_db(self.conf["1"]["db"], commisionWallet)
        balanceRunnerA = db.get_balance_from_db_by_pub(self.conf["1"]["db"], pubRunner)
    ...
    def test_deactive_contract_with_err(self):
        ...
        commisionWallet = db.get_commission_wallet(self.conf["1"]["db"], 1)
        balanceCommisionB = db.get_balance_from_db(self.conf["1"]["db"], commisionWallet)
        commission = db.get_system_parameter(self.conf["1"]["db"], "commission_size")
        tokenRunner, uid = actions.get_uid(self.conf["2"]["url"])
        signature = sign(self.keys["key2"], uid)
        pubRunner = get_public_key(self.keys["key2"])
        balanceRunnerB = db.get_balance_from_db_by_pub(self.conf["1"]["db"], pubRunner)
