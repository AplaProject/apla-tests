...
class TestCost():
    ...
    def bind_wallet(self):
        data_creater = actions.login(
            self.conf[1]["url"], self.conf[0]["private_key"], 0)
        token_creater = data_creater["jwtToken"]
        id = actions.get_contract_id(
            self.conf[1]["url"], "CostContract", token_creater)
    ...
    def test_bind_wallet(self):
        ...
        if node == 0:
            ...
            dict_expect = dict(balanceRunner=int(balance_runner_b),
                               platformBalance=int(
                                   b_node_balance[0]) - int(node_commission),
                               nodeBalance=int(
                                   b_node_balance[node]) + int(node_commission),
                               summ=int(summ_after),
                               history=True)
